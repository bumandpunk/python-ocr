...
class DataGrid: 
    ...
    def create_widgets(self, parent):
        """创建数据表格界面"""
        self.frame = ttk.Frame(parent, width=800)  # 增加到800宽度
    ...
    def delete_row(self, row_index):
        ...
        # 保存所有已修改的数据
        self.save_all_entered_values()
        
        # 确保索引有效
        if 0 <= row_index < len(self.app.detection_items):
            # 从数据源中删除
            del self.app.detection_items[row_index]
            ...
            # 自动选择上一行或下一行
            if row_index > 0:
                self.app.selected_index = row_index - 1
            elif len(self.app.detection_items) > 0:
    ...
    def add_row(self):
        ...
        self.update_data(self.app.detection_items, self.filename_label.cget("text").replace("当前文件: ", ""))
